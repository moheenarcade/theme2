...
import ProductDetailsComponent from "../../../components/homePageComponent/productDetails/index"
...
const ProductDetails = ({ slug }) => {
    ...
    console.log(selectedProductSku, "product in product selectedProductSku")
    ...
    useEffect(() => {
        async function fetchProduct() {
            try {
                const data = await getProductBySlug(finalSlug);
                if (!data) {
                    router.push("/not-found");
                    return;
                }
                setProduct(data.data);
                setProductSku(data.data?.product_sku);
            } catch (err) {
    ...
    useEffect(() => {
        async function fetchProduct() {
            try {
                const data = await getProductBySlug(productSku);
                if (!data) {
                    return;
                }
                setSelectedProductSku(data.data);
            } catch (err) {
                console.error("Failed to fetch product:", err);
            } finally {
                setLoading(false);
        ...
        <>
            ...
            <div className='max-w-[1125px] mx-auto px-4 xl:px-0 my-8'>
                <div className="flex md:gap-6 flex-col lg:flex-row relative">
                    ...
                    <div className="product-slideImage w-full lg:w-[50%] lg:sticky lg:top-28 self-start h-fit" dir={isRTL === 'ar' ? 'rtl' : 'ltr'}>
                        <Swiper
                            spaceBetween={10}
                        ...
                        >
                            {product?.images?.map((imgObj, index) => (
                                <SwiperSlide key={index}>
                                    <img
                                        className="w-[120px] aspect-[2/2] mx-auto"
                                        src={getImageUrl(imgObj)}
                                        alt={`Thumb ${index + 1}`}
                                    />
                                </SwiperSlide>
                    ...
                    <div className="product-buy-form w-full lg:w-[50%]">
                        <h3 className="text-[#000000] text-[25px] font-[500] hidden lg:block">{product?.name || "no name"}</h3>
                        {/* <div className="flex justify-between md:justify-start gap-4 items-center pt-2">
                            {product?.prices?.length > 0 && (() => {
                                ...
                                if (
                                    ...
                                    return (
                                        <div className="flex justify-start gap-4 items-center pt-2">
                                            <p className="text-[#592404FF] font-[500] text-[24px]">
                                                {numericPrice.toLocaleString()} {currencyCode}
                                            </p>
                                        </div>
                    ...
                    return (
                      <p className="text-[#592404FF] font-[500] text-[24px]">
                        ...
                        <div className="product-form mt-6">
                            ...
                            <div className=" mt-6">
                                <hr className='mb-6' />
                                <p className='text-[#592404] font-[500] text-center text-[18px]'>{t('Joint_treatment_gel_or_ointment')}</p>
